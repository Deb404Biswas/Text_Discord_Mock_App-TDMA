...
try:
    ...
    async def create_guild(request: Request,db:database,user:current_user,guild_name:str):
        if not await db.user_find_one(user['user_id']):
        ...
        user_doc=await db.user_find_one(user['user_id'])
...
try:
    ...
    @limiter.limit("5/minute")
    async def delete_guild(guild_id: str,db:database, request: Request,user:current_user):
    ...
    async def transfer_guild_ownership(guild_id: str, new_owner_id: str, request: Request,db:database,user:current_user):
        ...
        for new_owner_role in new_owner_roles:
            if new_owner_role['guild_id'] == guild_id:
                ...
                await db.user_update_one(new_owner_id,update_user_doc)
                break
        ...
        for current_user_role in current_user_roles:
            if current_user_role["guild_id"]==guild_id:
                ...
                await db.user_update_one(current_user_id,update_user_doc)
                break
        update_guild_doc={
            "$set": {
...
try:
    ...
    async def get_guild_info(guild_id: str, request: Request,db:database,user:current_user):
...
try:
    ...
    @limiter.limit("5/minute")
    async def update_guild_name(guild_id: str, new_guild_name: str, request: Request,db:database,user:current_user):
        user_id=user['user_id']
        user_name=user['user_name']
...
try:
    ...
    @limiter.limit("10/minute")
    async def remove_user_from_guild(guild_id: str, member_user_id: str, request: Request,db:database,user:current_user):
        current_user_id=user['user_id']
        current_user_name=user['user_name']
        await ValidUserCheck(current_user_id,current_user_name,guild_id,"kick_member")
        member_user_doc=await db.user_find_one(member_user_id)
